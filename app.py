...
Session(app)
mydb=mysql.connector.connect(host="localhost",user="root",password="Suresh@416",db="spm")
app.secret_key=b'ER\x10\xcc\x16S'
@app.route("/")
def index():
...
def register():
    if request.method=="POST":
        ...
        stu_mail=request.form["email"]
        stu_pwd=request.form["pwd"]
        stu_cpwd=request.form["cpwd"]
        cursor=mydb.cursor(buffered=True)
        cursor.execute("select count(email) from student_reg where email=%s",[stu_mail])
        data=cursor.fetchone()[0]
        if data==0:
            otp=genotp()
            data={"otp":otp,"stu_mail":stu_mail,"stu_fname":stu_fname,"stu_lname":stu_lname,"stu_pwd":stu_pwd}
            subject="Verication otp for SPM Application" 
            body=f"Registration otp for spm Application {otp}"
            sendmail(to=stu_mail,subject=subject,body=body)
...
def verifyotp(data1):
    try:
        ...
        if request.method=="POST":
            ...
            if uotp==data1['otp']:
                cursor=mydb.cursor(buffered=True)
                cursor.execute('insert into student_reg(email,student_fname,student_lname,password) values(%s,%s,%s,%s)',[data1['stu_mail'],data1['stu_fname'],data1['stu_lname'],data1['stu_pwd']])
                mydb.commit()
                cursor.close()
...
def login():
    if session.get('email'):
        ...
        if request.method=="POST":
            ...
            try:
                cursor=mydb.cursor(buffered=True)
                cursor.execute("select email,password from student_reg where email=%s",[email])
...
def updatenotes(nid):
    if not session.get('email'):
        ...
        if request.method=="POST":
            ...
            mydb.commit()
            cursor.close()
            flash(f'notes {title} updates successfully')
